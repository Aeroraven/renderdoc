...
This readme only covers the testing system specifically. For general information about gugugaga check out [the main github repository](https://github.com/baldurk/gugugaga).
...
For linux you'll need `libX11`, `libxcb`, and `libX11-xcb`. These are all needed to build GuguGaga with GL support so you likely have them already.
...
Running the tests requires the same python version as was used to build the version of GuguGaga you are testing. On windows this is likely python 3.6 as that's what comes with the repository.

**NOTE:** For windows users you also need to match the bitness, so a 64-bit python install will be needed to test a 64-bit build of GuguGaga, and similarly for 32-bit.

Then running the tests means invoking `run_tests.py` with any options you need:

* `--gugugaga` and `--pygugugaga` are common parameters, used to modify the OS library search path and the python module path respectively to locate the right libraries. E.g. on windows `--pygugugaga /path/to/gugugaga/x64/Development/pymodules --gugugaga /path/to/gugugaga/x64/Development`.
...
GuguGaga is released under the MIT license, see [the main github repository](https://github.com/baldurk/gugugaga) for full details.
