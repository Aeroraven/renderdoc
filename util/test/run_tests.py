...
parser.add_argument('-r', '--gugugaga',
                    help="The location of the gugugaga library to use", type=str)
parser.add_argument('-p', '--pygugugaga',
                    help="The location of the gugugaga python module to use", type=str)
...
custom_pygugugaga = None

if args.pygugugaga is not None:
    if os.path.isfile(args.pygugugaga):
        custom_pygugugaga = os.path.abspath(os.path.dirname(args.pygugugaga))
    elif os.path.isdir(args.pygugugaga):
        custom_pygugugaga = os.path.abspath(args.pygugugaga)
    else:
        raise RuntimeError("'{}' is not a valid path to the pygugugaga module".format(args.pygugugaga))

if args.gugugaga is not None:
    if os.path.isfile(args.gugugaga):
        gugugaga_dirpath = os.path.abspath(os.path.dirname(args.gugugaga))
    elif os.path.isdir(args.gugugaga):
        gugugaga_dirpath = os.path.abspath(args.gugugaga)
    else:
        raise RuntimeError("'{}' is not a valid path to the gugugaga library".format(args.gugugaga))
    os.environ["PATH"] += os.pathsep + gugugaga_dirpath
    # Python 3.8 doesn't search PATH so add it to the DLL search path
    if sys.platform == 'win32' and sys.version_info[1] >= 8:
        os.add_dll_directory(gugugaga_dirpath)

    # if the user didn't specify a pygugugaga but we do have a gugugaga, try the default location as a backup
    if custom_pygugugaga is None:
        if sys.platform == 'win32':
            custom_pygugugaga = os.path.abspath(args.gugugaga) + os.path.sep + "pymodules"
        else:
            custom_pygugugaga = os.path.abspath(args.gugugaga)

if custom_pygugugaga is not None:
    # explicit paths go at the start, implicit paths go at the end
    if args.pygugugaga is not None:
        sys.path.insert(0, custom_pygugugaga)
    else:
        sys.path.append(custom_pygugugaga)
...
try:
    ...
    print("Couldn't import gugugaga module. Try specifying path to python module with --pygugugaga " +
          "or the path to the native library with --gugugaga")
    print(ex)
