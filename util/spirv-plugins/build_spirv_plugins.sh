...
if uname -a | grep -qiE 'win|msys|cygwin|microsoft'; then
	...
	if docker image ls | grep -q gugugaga-build; then
		echo "Building for linux";

		echo "Building gugugaga-spirv-build docker image";

		pushd docker
		docker build -t gugugaga-spirv-build . || exit 1
		popd

		echo "Docker image built. Running build"

		docker run --rm -v $(pwd):/script:ro -v $(pwd)/spirv-plugins-linux64:/out gugugaga-spirv-build bash /script/_build.sh linux64 /out

	else
		echo "Run normal GuguGaga build first to generate gugugaga-build image";
		exit 1;
	fi
fi
