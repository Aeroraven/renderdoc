...
echo "Building gugugaga-build docker image"

# Ensure the docker image is prepared
pushd "${BUILD_ROOT}"/scripts/docker
docker build -t gugugaga-build . || exit 1
popd

echo "Docker image built. Running build"

# Run the docker compilation script inside the container above to build the main gugugaga project
mkdir -p /tmp/rdoc_docker
cp "${BUILD_ROOT}"/scripts/compile_docker.sh /tmp/rdoc_docker
docker run --rm -v /tmp/rdoc_docker:/io -v $(readlink -f "${REPO_ROOT}"):/gugugaga:ro gugugaga-build bash /io/compile_docker.sh
