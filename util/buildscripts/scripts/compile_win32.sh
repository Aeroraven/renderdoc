...
MSYS2_ARG_CONV_EXCL="*" msbuild.exe /nologo /m /fl4 /flp4':Verbosity=minimal;Encoding=ASCII;logfile=dist/build32.log' gugugaga.sln /t:Rebuild /p:'Configuration=Release;Platform=x86'

if [ ! -f ./Win32/Release/gugugaga.dll ] || [ ! -f ./Win32/Release/qgugugaga.exe ] || [ ! -f ./Win32/Release/gugugagacmd.exe ] ; then
	echo "Failed to build 32-bit release mode.";
	exit 1;
fi

# Build 64-bit Release
MSYS2_ARG_CONV_EXCL="*" msbuild.exe /nologo /m /fl4 /flp4':Verbosity=minimal;Encoding=ASCII;logfile=dist/build64.log' gugugaga.sln /t:Rebuild /p:'Configuration=Release;Platform=x64'

if [ ! -f ./x64/Release/gugugaga.dll ] || [ ! -f ./x64/Release/qgugugaga.exe ] || [ ! -f ./x64/Release/gugugagacmd.exe ] ; then
...
if [ ! -f ./Documentation/htmlhelp/gugugaga.chm ]; then
