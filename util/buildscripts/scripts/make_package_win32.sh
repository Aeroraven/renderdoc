...
if [ -d plugins-win64 ]; then
	...
	echo "WARNING: x64 plugins missing, download and extract https://gugugaga.org/plugins.zip in root";
...
if [ -d plugins-win32 ]; then
	...
	echo "WARNING: x86 plugins missing, download and extract https://gugugaga.org/plugins.zip in root";
...
if ls build-android-* > /dev/null; then
	find build-android-* -iname 'org.gugugaga.gugugagacmd.*.apk' -exec cp '{}' dist/Release64/plugins/android ';'
else
...
# In the 64bit release folder, make an x86 subfolder and copy in gugugaga 32bit
mkdir -p dist/Release64/x86
cp -R dist/Release32/{d3dcompiler_47.dll,gugugaga.dll,gugugaga.json,gugugagashim32.dll,gugugagacmd.exe,dbghelp.dll,symsrv.dll,symsrv.yes} dist/Release64/x86/
mkdir -p dist/ReleasePDBs64/x86
cp -R dist/ReleasePDBs32/{d3dcompiler_47.dll,gugugaga.dll,gugugaga.json,gugugaga.pdb,gugugagashim32.dll,gugugagashim32.pdb,gugugagacmd.exe,gugugagacmd.pdb,dbghelp.dll,symsrv.dll,symsrv.yes} dist/ReleasePDBs64/x86/

VERSION=`grep -E "#define GUGUGAGA_VERSION_(MAJOR|MINOR)" gugugaga/api/replay/version.h | tr -dc '[0-9\n]' | tr '\n' '.' | grep -Eo '[0-9]+\.[0-9]+'`

export GUGUGAGA_VERSION="${VERSION}"

# Ensure this variable passes through to windows on WSL
export WSLENV=$WSLENV:GUGUGAGA_VERSION
